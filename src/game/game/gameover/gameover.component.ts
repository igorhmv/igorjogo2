...
export class GameoverComponent extends Phaser.Scene implements OnInit {
  ...
  create(){
    this.add.text(phaserGame.scale.width/2-200, 10+this.delta, 'Fim de Jogo', { font: '50px Arial',color:'#000000' });
    this.add.text(phaserGame.scale.width/2-200, 100+this.delta, 'Pontuação: '+MainScene.getScore().toString(), { font: '30px Arial',color:'#000000' });
    this.tryAgainIcon = this.add.image(phaserGame.scale.width/2,200+this.delta,'tryAgain').setName("tryAgain").setInteractive();
    this.giveUpIcon = this.add.image(phaserGame.scale.width/2,400+this.delta,'giveUp').setName("giveUp").setInteractive();
