...
export class CovidBookComponent extends Phaser.Scene implements OnInit {
  ...
  create(){
    ...
    this.backIcon = this.add.image(width/2,600,'back').setName("back").setInteractive();
      ...
      this.add.text(phaserGame.scale.width/2-150, 10+this.delta, 'Livro COVID', { font: '50px Arial',color:'#000000' });

    this.add.text(phaserGame.scale.width/2-200,phaserGame.scale.height/8-20+this.delta,'Confira abaixo pequenos detalhes\nsobre as variantes do COVID-19:', { font: '20px Arial',color:'#000000' });
    this.add.image(phaserGame.scale.width/2-170,200+this.delta,'alpha');

    let infoAlpha="Alfa B.1.1.7\nDescoberta em: Reino Unido.\nEfeitos: Até 50% de aumento da\ntransmissão e potencialmente maior\ngravidade";

    this.add.text(phaserGame.scale.width/2-130,150+this.delta,infoAlpha.toString(),{ font: '20px Arial',color:'#000000' });

    let infoBeta="Beta B.1.351\nDescoberta em: África do Sul.\nEfeitos: Pensa-se que seja\naté 50% de aumento da transmissão.";

    this.add.image(phaserGame.scale.width/2-170,300+this.delta,'beta');
    this.add.text(phaserGame.scale.width/2-130,270+this.delta,infoBeta.toString(),{ font: '20px Arial',color:'#000000' });

    let infoGamma = "Gamma P.1\nDescoberta em: Brasil e Japão.\nEfeitos: Estudos sugerem que o P1\né 1,7 a 2,4 vezes mais transmissível\ndo que a anterior.";
    this.add.image(phaserGame.scale.width/2-170,400+this.delta,'gamma');
    this.add.text(phaserGame.scale.width/2-130,370+this.delta,infoGamma.toString(),{ font: '20px Arial',color:'#000000' });


    let infoDelta = "Delta P.1\nDescoberta em: Índia.\nEfeitos: Mais transmissível do que\na deformação original em estudos\npreliminares.";
    this.add.image(phaserGame.scale.width/2-170,520+this.delta,'delta');
    this.add.text(phaserGame.scale.width/2-130,490+this.delta,infoDelta.toString(),{ font: '20px Arial',color:'#000000' });

    this.backIcon = this.add.image(phaserGame.scale.width/2,650+this.delta,'back').setName("back").setInteractive();
